...
class Student extends Lambdasian {
  ...
  listSubjects() {
    ...
    while (i < this.favSubjects.length - 1) {
      response += this.favSubjects[i] + ', ';
      i++;
    }
    return response + this.favSubjects[i] + '!';
  }
  PRAssignment(subject) {
    return `${this.name} has committed a PR for ${subject}`;
  }
  sprintChallenge(challenge) {
    return `${this.name} has begun sprint challenge on ${challenge}`;
  }
}
